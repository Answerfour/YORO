...
class FrameExtractorGUI(ttk.Frame):
    ...
    def _create_video_list(self, parent):
        list_frame = ttk.LabelFrame(parent, text="视频列表", padding="10")
        list_frame.pack(fill=tk.BOTH, expand=True)
        
        btn_bar = ttk.Frame(list_frame)
        btn_bar.pack(fill=tk.X, pady=(0, 10))
        
        ttk.Button(btn_bar, text="添加视频", command=self._add_videos).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_bar, text="添加文件夹", command=self._add_folder).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_bar, text="清空列表", command=self._clear_list).pack(side=tk.LEFT, padx=2)
        ...
        self.video_info_label = ttk.Label(list_frame, text="共 0 个视频", foreground="blue")
    ...
    def _create_settings(self, parent):
        ...
        ttk.Label(dir_row, text="输出目录:").pack(side=tk.LEFT, padx=5)
        self.output_dir_var = tk.StringVar()
        ttk.Entry(dir_row, textvariable=self.output_dir_var, width=25).pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        ttk.Button(dir_row, text="浏览", command=self._select_output_dir).pack(side=tk.RIGHT, padx=5)
        
        time_frame = ttk.LabelFrame(parent, text="时间范围 (秒)", padding="10")
        time_frame.pack(fill=tk.X, pady=5)
        
        time_row = ttk.Frame(time_frame)
        time_row.pack()
        ttk.Label(time_row, text="起始:").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(time_row, text="结束:").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(quick_frame, text="前10秒", command=lambda: self._set_time(0, 10)).pack(side=tk.LEFT, padx=2)
        ttk.Button(quick_frame, text="前30秒", command=lambda: self._set_time(0, 30)).pack(side=tk.LEFT, padx=2)
        ttk.Button(quick_frame, text="前60秒", command=lambda: self._set_time(0, 60)).pack(side=tk.LEFT, padx=2)
        ttk.Button(quick_frame, text="整段", command=self._set_full_time).pack(side=tk.LEFT, padx=2)
        
        param_frame = ttk.LabelFrame(parent, text="切帧参数", padding="10")
        param_frame.pack(fill=tk.X, pady=5)
        
        param_row = ttk.Frame(param_frame)
        param_row.pack()
        ttk.Label(param_row, text="采样帧率 (帧/秒):").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(format_row, text="图片格式:").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(format_row, text="JPEG质量:").pack(side=tk.LEFT, padx=5)
        ...
        naming_frame = ttk.LabelFrame(parent, text="文件命名设置", padding="10")
        naming_frame.pack(fill=tk.X, pady=5)
        
        mode_frame = ttk.Frame(naming_frame)
        mode_frame.pack(fill=tk.X, pady=5)
        ttk.Label(mode_frame, text="命名方式:").pack(side=tk.LEFT, padx=5)
        self.naming_mode_var = tk.StringVar(value="sequence")
        ttk.Radiobutton(mode_frame, text="序号命名", variable=self.naming_mode_var, value="sequence").pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(mode_frame, text="时间戳命名", variable=self.naming_mode_var, value="timestamp").pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(mode_frame, text="自定义前缀", variable=self.naming_mode_var, value="custom").pack(side=tk.LEFT, padx=5)
        
        self.example_label = ttk.Label(naming_frame, text="示例: 000001.jpg", foreground="blue")
    ...
    def _create_buttons(self, parent):
        ...
        self.extract_btn = ttk.Button(button_frame, text="▶ 开始批量切帧", command=self._start_extraction, width=15)
        self.extract_btn.pack(side=tk.LEFT, padx=5)
        
        ttk.Button(button_frame, text="停止", command=self._stop_extraction, width=8).pack(side=tk.LEFT, padx=5)
    ...
    def _add_folder(self):
        ...
        if folder:
            ...
            self._update_video_info_label()
            self.log_frame.log(f"从文件夹添加了 {added_count} 个视频")
    ...
    def _load_video_info_async(self, path):
        def load():
            try:
                ...
                self.after(0, lambda: self.log_frame.log("警告: 未安装OpenCV，无法加载视频信息"))
                return
            ...
            try:
                ...
                if cap.isOpened():
                    ...
                    self.after(0, lambda: self.log_frame.log(f"已加载: {os.path.basename(path)} (时长: {duration:.1f}s)"))
            except:
                pass
    ...
    def _update_video_info_label(self):
        ...
        self.video_info_label.config(text=f"共 {count} 个视频 | 总时长: {total_duration:.1f} 秒")
    
    def _select_output_dir(self):
        directory = filedialog.askdirectory(title="选择输出目录")
        if directory:
            self.output_dir_var.set(directory)
            self.log_frame.log(f"输出目录: {directory}")
    ...
    def _set_full_time(self):
        if self.video_list and self.current_video_index < len(self.video_list):
            ...
            if video_path in self.videos_info:
                ...
                self.log_frame.log(f"设置为完整时长: {duration:.1f}秒")
    
    def _start_extraction(self):
        if not self.video_list:
            messagebox.showerror("错误", "请先添加要处理的视频")
            return
        ...
        try:
            ...
            messagebox.showerror("错误", f"输出目录不可写: {e}")
            return
    ...
    def _extraction_worker(self):
        ...
        for i, video_path in enumerate(self.video_list):
            ...
            self.after(0, lambda idx=i: self.current_video_label.config(text=f"处理视频 {idx + 1}/{total_videos}"))
        ...
        self.after(0, lambda: self.extraction_running and self.log_frame.log(f"全部完成！共提取 {total_frames_extracted} 帧"))
        self.extraction_running = False
    
    def _stop_extraction(self):
        if self.extraction_running:
            self.extraction_running = False
            self.log_frame.log("⚠ 停止请求")
